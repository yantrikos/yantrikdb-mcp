...
def remember(
    ...
    if memory_type not in valid_types:
        return _err(f"memory_type must be one of {valid_types}, got '{memory_type}'")
...
def recall(
    ...
    if feedback_rid:
        if feedback not in ("relevant", "irrelevant"):
            return _err("feedback must be 'relevant' or 'irrelevant'")
        with lock:
            db.recall_feedback(
    ...
    for r in response["results"]:
        items.append({
            "rid": r["rid"], "text": r["text"], "type": r["type"],
            "score": round(r["score"], 4), "importance": r["importance"], "created_at": r["created_at"],
            "scores": {k: round(v, 4) if isinstance(v, (int, float)) else v for k, v in r["scores"].items()},
...
def forget(
    ...
    if not rid and not rids:
        return _err("rid or rids required")
...
def correct(
    ...
    if not new_text or not new_text.strip():
        return _err("new_text must be non-empty")
...
def memory(
    ...
    """Manage individual memories — get, list, update importance, archive, or hydrate.
    ...
    Args:
        action: One of "get", "list", "update_importance", "archive", "hydrate".
        rid: Memory ID (required for get/update_importance/archive/hydrate).
        importance: New importance 0.0-1.0 (for update_importance).
        limit: Max results for list (default 50, max 200).
        offset: Pagination offset for list.
        domain: Filter for list.
        memory_type: Filter for list: "semantic", "episodic", "procedural".
        namespace: Filter for list.
        sort_by: For list: "created_at", "importance", "last_access".
    """
    valid = ("get", "list", "update_importance", "archive", "hydrate")
    if action not in valid:
        return _err(f"action must be one of {valid}")

    db, lock = _get_db(ctx)

    if action == "get":
        if not rid:
            return _err("rid required for get")
        with lock:
            mem = db.get(rid)
        if mem is None:
    ...
    if action == "update_importance":
        if not rid:
            return _err("rid required")
        if importance is None:
            return _err("importance required")
        with lock:
            mem = db.get(rid)
            if mem is None:
                return _err("Memory not found", rid=rid)
...
def graph(
    ...
    if action == "relate":
        if not entity or not target:
            return _err("entity and target required for relate")
        with lock:
    ...
    if action == "link":
        if not rid or not entity:
            return _err("rid and entity required for link")
        try:
            with lock:
    ...
    if action == "search":
        if not pattern:
            return _err("pattern required for search")
        with lock:
...
def conflict(
    ...
    if not conflict_id:
        return _err("conflict_id required")
    ...
    if action == "resolve":
        ...
        if strategy not in valid_strategies:
            return _err(f"strategy must be one of {valid_strategies}")
        if strategy == "merge" and (not new_text or not new_text.strip()):
            return _err("new_text required for merge strategy")
        try:
            if strategy == "dismiss":
                with lock:
    ...
    if action == "reclassify":
        if not new_type:
            return _err("new_type required for reclassify")
        with lock:
...
def trigger(
    ...
    # Lifecycle actions
    if not trigger_id:
        return _err("trigger_id required")
    action_map = {
...
def temporal(
    ...
    if action not in ("stale", "upcoming"):
        return _err("action must be 'stale' or 'upcoming'")
...
def procedure(
    ...
    if action not in ("learn", "surface", "reinforce"):
        return _err("action must be 'learn', 'surface', or 'reinforce'")
    ...
    if action == "reinforce":
        if not rid:
            return _err("rid required")
        if outcome is None:
            return _err("outcome required (0.0-1.0)")
        with lock:
...
def category(
    ...
    if not category_name:
        return _err("category_name required")
    ...
    if action == "learn":
        if not members:
            return _err("members required as [[token, confidence], ...]")
...
def personality(
    ...
    if action == "get":
        with lock:
            profile = db.derive_personality() if recompute else db.get_personality()
        return json.dumps(profile)

    if not trait_name or score is None:
        return _err("trait_name and score required for set")
    with lock:
...
def stats(
    ...
    if action == "maintenance":
        ...
        if maintenance_op not in valid_ops:
            return _err(f"maintenance_op must be one of {valid_ops}")
        t0 = time.time()
        with lock:
